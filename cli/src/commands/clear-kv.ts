  ...
  try {
    ...
    while (hasMore) {
      ...
      const listOutput = execSync(
        "npx wrangler kv key list --binding=SORARE_AI_DATA --preview false",
        { encoding: "utf-8" }
      );
      ...
      const tempFile = path.join(
        process.cwd(),
        `temp-keys-to-delete-${Date.now()}.json`
      );
      ...
      execSync(
        `npx wrangler kv bulk delete ${tempFile} --binding=SORARE_AI_DATA --preview false --force`,
        { stdio: "inherit" }
      );
